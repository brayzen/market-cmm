...
{if isset($smarty.get.action)}
    ...
    {if $smarty.get.action == 'add' || $smarty.get.action == 'edit'}
        ...
        function cat_chooser(cat_id, cat_name){
            ...
            $.getJSON(rlUrlHome+'request.ajax.php', {item: 'getCategoryPathByID', id: cat_id}, function(path){
                path = cat_id == 0 ? '' : path+'/';
                ...
                // edit category mode
                    ...
                    var current_path = $('input[name=path]').val().split('/');
                    var tail = current_path[current_path.length - 1];

                    $('#ap').html(path);
                    $('input[name=path]').val(tail);
                }
            });
        }
                ...
                <tr>
                    <td>
                        <span style="padding: 0 5px 0 0;" class="field_description_noicon">{strip}
                            <span id="abase">{if $abase}{$abase}{else}{$smarty.const.RL_URL_HOME}{/if}</span>
                            <span id="ab">{if $listing_types[$type].Links_type == 'full' && $pages.$type_page}{$pages.$type_page}/{/if}</span>
                            <span id="ap"></span>
                        {/strip}</span>
                    </td>
