...
{if !$deny}
    ...
    {elseif $smarty.get.action == 'view'}
                    ...
                    {if $photos}
                        ...
                        {foreach from=$photos item='photo' name='photosF'}
                            <li {if $smarty.foreach.photosF.iteration%2 != 0}class="nl"{/if}>
                                <a title="{$photo.Description}" rel="group" href="{$smarty.const.RL_FILES_URL}{$photo.Photo}"><img alt="" class="shadow" src="{$smarty.const.RL_FILES_URL}{$photo.Thumbnail}" /></a>
                            </li>
                        {/foreach}
                        </ul>
                    {/if}
        ...
        <div class="tab_area map listing_details hide">
            ...
            <script type="text/javascript" src="//maps.googleapis.com/maps/api/js?language={$smarty.const.RL_LANG_CODE}&key={$config.google_map_key}"></script>
            <script type="text/javascript" src="//www.google.com/uds/api?file=uds.js&v=1.0&key={$config.google_map_key}"></script>
