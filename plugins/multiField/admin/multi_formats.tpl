...
{else}
                ...
                <td class="field">
                    <span class="field_description_noicon" style="padding: 0;">{strip}
                        ...
                        {if $parent_path}
                            {$parent_path}
                        {else}
                            <input type="text" id="ni_sub_path" value="{$parent_path}" />
                        {/if}
                        .{$domain_info.host}
                        {if $config.mf_geo_subdomains_type == 'mixed' && $head_level != $smarty.get.parent}
                            /<input type="text" id="ni_path" value="{$item.Path}" />
                        {/if}
                        /
                    {/strip}</span>
                </td>
                ...
                <td class="field">
                    <span class="field_description_noicon" style="padding: 0;">{strip}
                        ...
                        {if $parent_path}
                            {$parent_path}/
                        {/if}
                        <input type="text" id="ni_path" style="width: 200px;" value="{$item.Path}" />
                    {/strip}</span>
                </td>
            {/if}
        </tr>
    ...
    {/literal}
        ...
        xajax_addItem( $('#ni_key').val(), names, $('#ni_status').val(), '{$smarty.get.parent}'{if $geo_filter}, $('#ni_path').val(), $('#ni_sub_path').val(){/if});
    {literal}
    }
    </script>
    {/literal}
    ...
    {/literal}
        ...
        xajax_editItem(key, names, $('#ei_status').val(), '{$smarty.get.parent}'{if $geo_filter}, $('#ei_path').val(), $('#ei_sub_path').val(){/if});
    {literal}
    }
    </script>
    {/literal}
    ...
    {if $smarty.get.parent}
        ...
        {literal}
        $(document).ready(function(){
            {/literal}{if !$level || $parent_default}{literal}
            ...
            itemsGrid = new gridObj({
                ...
                fields: [
                    ...
                    {/literal}{if $geo_filter && !$config.mf_geo_subdomains}{literal}
