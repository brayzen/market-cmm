...
<ul class="select-type{if $dropdown_data|@count <= 1} hide{/if}">
    {foreach from=$dropdown_data item='dropdown_item' name='sectionsF'}{strip}
        ...
        <li>
            <label>
                <input type="radio" name="section{if $section_postfix}_{$section_postfix}{/if}" value="{$dropdown_item.Key}" data-path="{$pages[$dropdown_item.Page_key]}" data-no-user-category="{if $no_user_category || !$dropdown_item.Cat_custom_adding}true{else}false{/if}" /> {$dropdown_item.name}
            </label>
        </li>
    {/strip}{/foreach}
</ul>
