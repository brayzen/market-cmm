    ...
    <div class="content-padding">
        ...
        {if $config.membership_module}
            <div id="area_membership" class="tab_area {if $smarty.request.info != 'membership'}hide{/if}">
                                    ...
                                    <div class="renew">
                                        ...
                                        {else}
                                            {if $membership_plan.Limit > $account_info.plan.Count_used || $membership_plan.Limit == 0}
                                                <a class="button" href="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/renew.html?info=membership{else}?page={$pageInfo.Path}&amp;renew&amp;info=membership{/if}">{if $account_info.Payment_status == 'unpaid'}{$lang.proceed_checkout}{else}{$lang.renew}{/if}</a>
                                            {/if}
                                        {/if}                                                 
                                    </div>
                                {/if}
                ...
                {if $step != 'renew'}
                    <a href="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/purchase.html{else}?page={$pageInfo.Path}&amp;purchase{/if}" class="button">{if $membership_plan}{$lang.change_plan}{else}{$lang.select_plan}{/if}</a>
                {else}
                    <form id="form-checkout" name="payment" method="post" action="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/renew.html?info=membership{else}?page={$pageInfo.Path}&renew=&info=membership{/if}">
