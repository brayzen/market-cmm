			...
			{foreach from=$news item='news_item'}
				<li>
					<div>
						...
						<a class="link-large" title="{$news_item.title}" href="{$rlBase}{if $config.mod_rewrite}{$pages.news}/{$news_item.Path}.html{else}?page={$pages.news}&amp;id={$news_item.ID}{/if}"><h4>{$news_item.title}</h4>
						{rlHook name='newsPostCaption'}</a>
					</div>
					
					<article>
						{$news_item.content|strip_tags:false|truncate:$config.news_page_content_length:"":false}{if $news_item.content|strlen > $config.news_page_content_length}...{/if}
						{rlHook name='newsPostContent'}
					</article>
				</li>
			{/foreach}
