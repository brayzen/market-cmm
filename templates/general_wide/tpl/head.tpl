...
<body class="large {$pageInfo.Key|replace:'_':'-'}-page{if !$side_bar_exists} no-sidebar{/if}{if $bread_crumbs|@count > 2 && $pageInfo.Key != 'home'} bc-exists{/if}{if $config.header_banner_space} header-banner{/if}{if $pageInfo.Controller == 'listing_details' && $blocks.get_more_details} get-details-box{/if}{if $config.general_simple_color != 'green'} {$config.general_simple_color}-theme{/if}{if !$config.img_crop_thumbnail} listing-fit-contain{/if}" {if $smarty.const.RL_LANG_DIR == 'rtl'}dir="rtl"{/if}>
