...
class rlAdmin extends reefless
{
        ...
        foreach ($configs as $key => &$value) {
            ...
            switch ($value['Key']) {
                ...
                case 'banner_in_grid_position':
                    ...
                    for ($i = 1; $i <= ((int) $GLOBALS['config']['listings_per_page'] ?: 10); $i++) {
                        $value['Values'][$i] = $i;
                    }
                    break;
     ...
     * @package xAjax
     *
     * @param string $ids     - listings ids, ex: 250,3731,501,...
    ...
    public function apNotifications()
    {
        ...
        if (!$config['google_map_key'] || !$config['google_server_map_key']) {
