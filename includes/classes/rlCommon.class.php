...
class rlCommon extends reefless
{
    ...
    {
        global $rlSmarty, $fields, $main_menu, $tpl_settings, $account_info, $deny_pages, $config, $account_menu, $footer_menu;
        ...
        $menus = $GLOBALS['rlLang']->replaceLangKeys($menus, 'pages', array('name', 'title'));
     ...
     * Hreflang Tags
     *
     * Hreflang Tags - add hreflang meta tags to the header
     *
     * @since 4.6.0
     **/
    public function hreflangTags()
    {
        global $languages, $page_info, $config, $currentPage, $rlValid, $rlSmarty;

        if (count($languages) == 1) {
            return;
        }

        $url_home = RL_URL_HOME;
        $hreflang = array();

        // Replace url home if current page is account details with address on subdomain
        if ($page_info['Controller'] == 'account_type'
            && $config['mod_rewrite']
            && $config['account_wildcard']
        ) {
            $url_home = str_replace($rlValid->getDomain($url_home), $_SERVER['HTTP_HOST'], $url_home);
        }

        foreach ($languages as $code => $lang_item) {
            if ($page_info['Key'] == 'view_details') {
                $url = $GLOBALS['reefless']->getListingUrl($GLOBALS['listing_data'], $code);
            } else {
                $url = $url_home;
                if ($config['mod_rewrite']) {
                    $url .= $lang_item['dCode'] . $currentPage;
                } else {
                    $url .= '?language={$lang_item.Code}';
                }
            }
        ...
        $rlSmarty->assign_by_ref('hreflang', $hreflang);
    }
    ...
    private function metaPagination()
    {
        ...
        if ($_GET['pg']) {
            $postfix = str_replace('{page}', $_GET['pg'], $GLOBALS['lang']['title_page_part']);
     ...
     **/
    public function buildPagingUrlTemplate($add_url = '', $custom = '', $method = false, $var = false)
    {
