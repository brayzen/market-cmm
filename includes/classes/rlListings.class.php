...
class rlListings extends reefless
{
    ...
    public function getListings($category_id = false, $order = false, $order_type = 'ASC', $start = 0, $limit = 10, $listing_type = false)
    {
        ...
        $sql .= "`T1`.*, `T3`.`Path` AS `Path`, `T3`.`Key` AS `Key`, `T3`.`Type` AS `Listing_type`, `T3`.`Parent_keys`, `T3`.`Parent_IDs`, ";
    ...
    public function getMyFavorite($order = 'ID', $order_type = 'asc', $start = 0, $limit = false)
    {
        ...
        $sql .= "`T1`.*, `T4`.`Path`, `T4`.`Type` AS `Listing_type`, `T4`.`Key` AS `Key`, `T4`.`Parent_ID`, `T4`.`Key` AS `Cat_key`, `T4`.`Parent_IDs`, ";
    ...
    {
        global $rlValid, $category, $rlHook, $rlCommon, $rlDb;
        ...
        $sql .= "`T1`.*, `T4`.`Path`, `T4`.`Parent_ID`, `T4`.`Type` AS `Listing_type`, `T4`.`Parent_keys`, `T4`.`Parent_IDs` ";
    ...
    public function originalUrlRedirect($mode = 'listing', $data = array())
    {
        ...
        switch ($mode) {
            ...
            case 'listing':
                ...
                if (RL_DIR) {
                    $rlDir = str_replace("\\", "/", RL_DIR); //for windows servers
                    $pattern = '^(' . str_replace('/', '\/', $rlDir) . ')';
    ...
    public function prepareListings(&$listings, $count = false)
    {
        ...
        foreach ($listings as &$listing) {
            ...
            $out_listing['lu'] = $GLOBALS['reefless']->getListingUrl($listing);
