...
if (!empty($listing_id)) {
    ...
    // browse/quick search mode
        ...
        if ($category) {
            ...
            $rlListings->originalUrlRedirect('category', $category['ID']);
        }
        ...
        if (is_numeric($category['ID'])) {
            ...
            // get listings
            $listings = $rlListings->getListings($category['ID'], $order_field, $sort_type, $pInfo['current'], $config['listings_per_page'], $listing_type['Key']);
            $rlSmarty->assign_by_ref('listings', $listings);
