...
if (defined('IS_LOGIN')) {
    ...
    if ($config['membership_module']) {
        ...
        if ($step != 'purchase') {
            $sql = "SELECT *, COUNT(*) AS `count` FROM `{db_prefix}membership_plans` ";
            $sql .= "WHERE `Status` = 'active' AND FIND_IN_SET('{$account_info['Type']}', `Allow_for`) > 0";
            $ms_plans_total = $rlDb->getRow($sql);
            $rlSmarty->assign('ms_plans_total', (int) $ms_plans_total['count']);
        }
    }
