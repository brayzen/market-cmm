...
if ($_GET['renew']) {
    ...
    if (!$errors) {
        ...
        if (!$rlPayment->isPrepare()) {
            ...
            ? $page_info['Path'] . '.html?canceled'
            : '?page=' . $page_info['Path'] . '&canceled';

            $success_url = SEO_BASE;
            $success_url .= $config['mod_rewrite']
            ? $page_info['Path'] . '.html?completed'
            : '?page=' . $page_info['Path'] . '&completed';
    ...
    if ($_POST['action'] == 'submit') {
        ...
        if (!$errors) {
            ...
            if ($plan_info['Price'] > 0) {
                ...
                $cancel_url = SEO_BASE;
                $cancel_url .= $config['mod_rewrite']
                ? $pages['my_packages'] . '.html?canceled'
                : 'index.php?page=' . $pages['my_packages'] . '&amp;canceled';

                $success_url = SEO_BASE;
                $success_url .= $config['mod_rewrite']
                ? $pages['my_packages'] . '.html?completed'
                : 'index.php?page=' . $pages['my_packages'] . '&amp;completed';
